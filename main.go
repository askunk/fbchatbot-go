...
func main() {
	...
	fmt.Println("Starting serve on :9090")
...
func MessengerVerify(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" { // {{{
		...
		if len(verify_token) > 0 && len(challenge) > 0 && verify_token == "PUTYOURTOKENHERE" {
		...
		if err := json.NewDecoder(r.Body).Decode(input); err == nil {
			...
			if resp, err := getApiAiResponse(*input); err == nil {
				...
				reply.Message.Seq = 0 // these fields are not used so remove them without omit empty
				reply.Message.Mid = ""

				b, _ := json.Marshal(reply)
				url := fmt.Sprintf("https://graph.facebook.com/v.2.10/me/messages?access_token=%s", PAGE_TOKEN)
				http.Post(url,
					"application/json",
					bytes.NewReader(b))
			}
...
func getApiAiResponse(m MessengerInput) (resp string, err error) {
	...
	url := fmt.Sprintf("https://api.api.ai/v1query?V=20160518&lang=En&%s", params.Encode())
